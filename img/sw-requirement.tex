...
\begin{document}
  ...
  \begin{tikzpicture}
    ...
    \node[
      ...
      \includegraphics[height=3cm,page=1]
        {sw-requirement-res-2.pdf}\\スマートフォン
    ...
    \node[
      ...
      \includegraphics[height=3cm,page=1]
        {sw-requirement-res-4.pdf}\\処理
    };
    \node[
      font=\tiny,
      align=center] at (\btx, \bty + 5mm) {
      \includegraphics[height=3cm,page=1]
        {sw-requirement-res-3.pdf}\\サーバ
    };
  ...
  \begin{tikzpicture}
    ...
    \node[align=center,
      anchor=north,
      font=\tiny,
      yshift=-1.8cm] at (\atx, \cty) {現行サーバ};

    \node[
      yshift=0mm,
      anchor=center] at (\btx, \cty) {
      \includegraphics[page=1]
        {sw-requirement-res-8.pdf}
    };
    ...
    \node[align=center,
      anchor=north,
      font=\tiny,
      yshift=-1.8cm] at (\btx, \cty) {置き換えサーバ};

    \node[font=\tiny,
      yshift=1.8cm,
      single arrow,
      draw] at (\ctx, \cty) {変更なしで実行可能？};
    
    \begin{scope}[yshift=-3mm]
      \newcommand{\txtboxw}{46mm}
      \newcommand{\txtboxh}{28mm}
      \path [clip,draw, rounded corners=1mm]
        (\ctx - \txtboxw / 2, \cty - \txtboxh / 2)
        rectangle
        ++(\txtboxw, \txtboxh);
      \node [anchor=center,xshift=10mm] at (\ctx, \cty) {
        \includegraphics[height=3cm,page=1]
          {sw-requirement-res-6.pdf}
      };
    \end{scope}
